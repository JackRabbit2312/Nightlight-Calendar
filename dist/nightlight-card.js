...
class NightlightDashboard extends LitElement {
  ...
  _toggleChore(entityId) {
    const state = this.hass.states[entityId].state === 'on' ? 'off' : 'on';
    this.hass.callService('input_boolean', state === 'on' ? 'turn_on' : 'turn_off', { entity_id: entityId });
  }
        ...
        <div class="main-scrollable-body">
            ...
            <div class="columns-grid" style="--cols: ${daysCount}">
              ${Array.from({length: daysCount}).map((_, i) => {
                ...
                return html`
                  <div class="day-col">
                    <div class="day-header-sticky">${d.toLocaleDateString('default', {weekday: 'short', day: 'numeric'})}</div>
